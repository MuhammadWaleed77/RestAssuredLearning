...
//SSL
		RequestSpecification req = new RequestSpecBuilder().setBaseUri("https://rahulshettyacademy.com")
				.setContentType(ContentType.JSON).build();   //content type username pass wala json ha

		LoginRequest loginRequest = new LoginRequest();
		loginRequest.setUserEmail("postman9@gmail.com");
		loginRequest.setUserPassword("1234567aA!");
		...
		//WIthout POJO COnvert into tring and get responce
	//	Response response = reqLogin.when().post("/api/ecom/auth/login").then().extract().response();
	...
	//	System.out.println(responseString);
		...
		RequestSpecification addProductBaseReq = new RequestSpecBuilder().setBaseUri("https://rahulshettyacademy.com")
				.addHeader("authorization", token).build();  //not sending any json so no need
		...
		RequestSpecification reqAddProduct = given().log().all().spec(addProductBaseReq).param("productName", "Laptop")
				...
				.multiPart("productImage", new File("/Users/numuqa/Downloads/Pictures/Chick.jpg"));  //add Image use multiPart
